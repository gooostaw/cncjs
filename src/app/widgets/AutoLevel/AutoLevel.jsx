...
import { MODAL_PREVIEW, MODAL_PREVIEW2 } from "./constants";
  ...
  render() {
    ...
    if (Object.prototype.hasOwnProperty.call(state, "probingData")) {
      ...
      if (state.status === 'running' && state.probingData.printed === false) {
        ...
        log.info(
          "AutoLevel.jsx result :" + JSON.stringify(state.probingData.result)
        );

        let sx =
          state.probingData.result.x ?? state.controller.state.status.wpos.x;
        let sy =
          state.probingData.result.y ?? state.controller.state.status.wpos.y;
          ...
          <div className="container-fluid">
            ...
            <div className="row">
              <button
                ...
                onClick={actions.startAutoLevel}
              >
                Probe autolevel
              </button>
            </div>
